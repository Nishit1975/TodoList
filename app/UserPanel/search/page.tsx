...
import { fetchUserSearchData } from './actions';
...
export default function SearchPage() {
  ...
  const fetchData = useCallback(async (
    ...
    setError(null);
    try {
      const data = await fetchUserSearchData(
        query,
        priority,
        status,
        assignee,
        ...
        <div className="mb-8">
          <div className="flex items-center gap-4 mb-2">
            ...
            <div>
              ...
              <p className="text-slate-500 font-medium mt-1">Search across all your tasks and projects</p>
            </div>
          </div>
        </div>
                    ...
                    {displayResults.map((item) => (
                      <div key={`${item.type}-${item.id}`}>
                        ...
                        {item.type === 'project' && 'name' in item && (
                          <Link
                            href={`/UserPanel/projects/${item.id}`}
